...
func _process(delta: float) -> void:
	...
	if _auraMaterial:
		...
		# +25 px desplaza el aura 25 píxeles hacia abajo sin mover el personaje.
		var center_px := Vector2(SUBVIEWPORT_SIZE.x / 2.0, _previewCharacter.position.y - _characterHeightPx * 0.5 + 25.0)
